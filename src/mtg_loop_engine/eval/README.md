...
| Prerequisite analysis | `classify.py` | Participation / assumptions / `strict_two_card` **detection**; discloses tokens, aura host, **scaled-mana board fodder**, and counter seeds as `generic_prerequisites`. **Known gap:** does not yet classify Path-b setup `seed_*` ops — merge with board generics when convenient (`ROADMAP.md` §2b) |
