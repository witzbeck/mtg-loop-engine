...
- `StaticCopyActivatedAbility` (Rings / Bracers): combo-favorable copy of non-mana activations when payment is available; mana abilities excluded.
- `CopyPendingTriggerEffect` (Strionic): duplicate the head pending trigger.
...
- `AdditionalCombatEffect`: bumps `extra_combat` (paired with untap on Assault / Moraug / Bloodthirster).
