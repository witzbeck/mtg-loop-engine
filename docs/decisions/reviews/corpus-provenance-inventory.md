...
| `oracle:training-grounds` | Training Grounds | False | **SYNTHETIC** (rename) or rebuild | Tuned cost-reduction; not creature-scoped {2}; Basalt+Grounds gold is not real MTG |
| `oracle:intruder-alarm` | Intruder Alarm | False | ORACLE_EXACT (candidate) | Verify vs snapshot (target wording) |
| `oracle:token-tapper` | Eager Apprentice | True | SYNTHETIC | Already honest fixture |
| `oracle:phyrexian-altar` | Phyrexian Altar | False | ORACLE_EXACT (candidate) | |
| `oracle:gravecrawler` | Gravecrawler | False | **ORACLE_DIVERGENT** → fix or SYNTHETIC | Activated return ≠ cast-from-GY + Zombie |
...
| `oracle:hardened-scales` | Hardened Scales | False | **ORACLE_DIVERGENT** → SYNTHETIC or true replacement | Invented activated put-counter |
| `oracle:reassembling-skeleton` | Reassembling Skeleton | False | **ORACLE_DIVERGENT** | Wrong cost / tapped return |
