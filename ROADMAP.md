...
2. **Grow COMPLETE pool (M5.2)** — prefer **self-starting** two-card physics (path **a**). **Slice 8+ is blocked on M5.1 frontier evidence** (not intuition). Curriculum ritual: [`docs/runbooks/M5_NOVEL_CANDIDATES.md`](docs/runbooks/M5_NOVEL_CANDIDATES.md).
   ...
   - **Slice 17+** ○ — choose from live frontier P0/P1; cite compact pair-unlock evidence in the curriculum PR.
...
3. **Workbench adjudication (M5.3)** ◐ — live queue after slices 14–16 remeasure (**63** COMPLETE · **57** verified · **51** in_reference · **6** absent): prior defender-mana + Sunbond/Triskelion pairs, plus **Heronblade Elite + Freed/Pemmin's** — all **`valid_generic_prerequisite`**, keep `ABSENT_FROM_REFERENCE` (not `NOVEL`). Run `--persist-workbench` after each curriculum PR.
